...
//        saveMember(memberSaveRequest);
    ...
    void findAll() {
        // given
        saveMap(saveMapApi, new MapCreateUpdateRequest(smallHouse.getName(), smallHouse.getMapDrawing(), MAP_SVG));
    ...
    private ExtractableResponse<Response> findAllMaps(String api) {
        return RestAssured
                .given(getRequestSpecification()).log().all()
                .accept("application/json")
                .header("Authorization", AuthorizationExtractor.AUTHENTICATION_TYPE + " " + accessToken)
